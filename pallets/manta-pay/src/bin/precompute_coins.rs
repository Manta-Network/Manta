...
fn main() -> Result<()> {
    ...
    for i in 0..1 {
        ...
        let (to_public_input, to_public) = sample_to_public(
            ...
            unchecked_account_id::<sp_core::sr25519::Public>("Charlie").into(),
            &mut rng,
        );
    ...
    let mut file = File::create("./precomputed-1-iterations/precomputed_mints_v6")?;
    file.write_all(&<[TransferPost]>::encode(&mints))?;
    // let mut file = File::create("./precomputed-1-iterations/precomputed_transfers_v6")?;
    // file.write_all(&<[TransferPost]>::encode(&transfers))?;
    let mut file = File::create("./precomputed-1-iterations/precomputed_reclaims_v6")?;
