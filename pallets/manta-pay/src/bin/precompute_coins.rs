...
fn main() -> Result<()> {
    ...
    let mut to_privates = Vec::new();
    let mut private_transfer_inputs = Vec::new();
    let mut private_transfers = Vec::new();
    let mut to_public_inputs = Vec::new();
    let mut to_publics = Vec::new();
    for i in 0..2 {
        let mut utxo_accumulator = UtxoAccumulator::new(utxo_accumulator_model.clone());
        let asset_id = if i == 0 { 1.into() } else { 8.into() };
    ...
    for i in 0..2 {
        ...
        private_transfer_inputs.push(private_transfer_input[0].clone());
        private_transfer_inputs.push(private_transfer_input[1].clone());
        private_transfers.push(private_transfer);
    }
    ...
    for i in 0..2 {
        ...
        to_public_inputs.push(to_public_input[0].clone());
        to_public_inputs.push(to_public_input[1].clone());
        to_publics.push(to_public);
    }
    ...
    write_const_nested_array!(target_file, TO_PRIVATE, to_privates)?;
    write_const_nested_array!(target_file, PRIVATE_TRANSFER_INPUT, private_transfer_inputs)?;
    write_const_nested_array!(target_file, PRIVATE_TRANSFER, private_transfers)?;
    write_const_nested_array!(target_file, TO_PUBLIC_INPUT, to_public_inputs)?;
    write_const_nested_array!(target_file, TO_PUBLIC, to_publics)?;
