...
#[inline]
fn main() -> Result<()> {
    let name = "precomputed_coins.rs";
    let target_file = env::args()
        .nth(1)
        .map(PathBuf::from)
        .unwrap_or(env::current_dir()?.join(name));
    assert!(
        !target_file.exists(),
    ...
    let (private_transfer_input, private_transfer) = sample_private_transfer(
        ...
        &parameters,
        &utxo_accumulator_model,
        AssetId(asset_id).value(10),
        AssetId(asset_id).value(20),
        &mut rng,
    );
    ...
    write_const_nested_array!(target_file, MINT, mints)?;
