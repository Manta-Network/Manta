...
fn main() -> Result<()> {
    ...
    let mut file = File::create("./precomputed-10-iterations/precomputed_mints_v3")?;
    file.write_all(&<[TransferPost]>::encode(&mints))?;
    let mut file = File::create("./precomputed-10-iterations/precomputed_transfers_v3")?;
    file.write_all(&<[TransferPost]>::encode(&transfers))?;
    let mut file = File::create("./precomputed-10-iterations/precomputed_reclaims_v3")?;
