...
use crate::{
    ...
    mock::{
        ...
        CollatorSelection, ExtBuilder, ParachainStaking, RuntimeEvent as MetaEvent, RuntimeOrigin,
        Test,
    },
...
// WHITELIST END SECTION Remove after whitelist period
...
fn locking_zero_amount_is_ignored() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            // Now the lock will be ignore if amount is 0 in polkadot-v1.0.0
            assert_eq!(crate::mock::query_lock_amount(1, DELEGATOR_LOCK_ID), None);
        });
}
