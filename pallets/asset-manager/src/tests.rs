...
use manta_primitives::{AssetRegistarMetadata, AssetLocation};
use xcm::{latest::prelude::*, VersionedMultiLocation};

#[test]
fn basic_setup_should_work(){
    new_test_ext().execute_with(||{
        assert!(AssetIdLocation::<Runtime>::iter_values().next().is_none())
    });
}
...
fn register_asset_should_work(){
    ...
    let source_location = AssetLocation(VersionedMultiLocation::V1(MultiLocation::parent()));
    let new_location = AssetLocation(VersionedMultiLocation::V1(
...
fn update_asset(){
    ...
    new_test_ext().execute_with(||{
        ...
        assert_noop!(AssetManager::update_asset_location(
            Origin::root(),
            1, 
            new_location.clone()),
            crate::Error::<Runtime>::UpdateNonExistAsset
        );
        assert_noop!(AssetManager::update_asset_metadata(
            Origin::root(),
            1, 
            new_metadata.clone()),
            crate::Error::<Runtime>::UpdateNonExistAsset
