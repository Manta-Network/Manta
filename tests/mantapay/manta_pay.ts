...
import {
  single_map_storage_key,
  double_map_storage_key,
  delay,
  HashType,
 ...
 */
export function generate_shards_entry(
  shard_idx: number,
  utxo_idx: number
): Uint8Array {
 ...
 */
function generate_batched_utxos(
  per_shard_amount: number,
  checkpoint: Array<number>
) {
  const data = [];
  for (
    let shard_idx = 0;
    shard_idx < manta_pay_config.shard_number;
    ++shard_idx
  ) {
    for (
      let utxo_idx = checkpoint[shard_idx];
      utxo_idx < checkpoint[shard_idx] + per_shard_amount;
      ++utxo_idx
    ) {
...
export async function setup_storage(
  ...
  for (
    let big_batch_idx = 0;
    big_batch_idx < config.utxo_big_batch_number;
    big_batch_idx++
  ) {
    ...
    const utxo_batch_done = await insert_utxos_in_batches(
      api,
      keyring,
      config.utxo_batch_number,
      config.utxo_batch_size_per_shard,
      receiver_checkpoint,
      250
    );
    console.log(
      ">>>> Complete %i big batch with %i UTXOs",
      big_batch_idx + 1,
      utxo_batch_done *
        config.utxo_batch_size_per_shard *
  ...
  console.log(
    ">>>> Inserting void numbers: %i per batch, %i batch",
    config.vn_batch_size,
    config.vn_batch_number
  );
  const vn_batch_done = await insert_void_numbers_in_batch(
    api,
    keyring,
    config.vn_batch_size,
    config.vn_batch_number,
  ...
  console.log(
    ">>>> Complete inserting %i void numbers",
    vn_batch_done * config.vn_batch_size
  );
}
...
export async function execute_with_root_via_governance(
  ...
  await api.tx.council
    .propose(
      1,
      encodedExternalProposeDefault,
      encodedExternalProposeDefault.length
    )
    .signAndSend(keyring, { nonce: -1 });
  let fastTrackCall = await api.tx.democracy.fastTrack(
    encodedCallDataHash,
    1,
    1
  );
  await api.tx.technicalCommittee
    .propose(1, fastTrackCall, fastTrackCall.encodedLength)
    .signAndSend(keyring, { nonce: -1 });
  await api.tx.democracy
    .vote(referendumIndexObject.referendumIndex, {
      Standard: {
