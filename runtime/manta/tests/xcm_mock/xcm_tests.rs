...
fn reserve_transfer_relaychain_to_parachain_a_then_back() {
    ...
    ParaA::execute_with(|| {
        ...
        assert_ok!(parachain::XTokens::transfer(
            ...
            xcm_simulator::Limited(weight_at_most)
        ));
    });
...
fn send_para_a_asset_to_para_b_with_insufficient_fee() {
    ...
    ParaA::execute_with(|| {
        assert_ok!(parachain::XTokens::transfer(
            ...
            amount,
            Box::new(VersionedMultiLocation::V1(dest)),
            xcm_simulator::Limited(dest_weight),
        ));
        assert_eq!(
            parachain::Balances::free_balance(&ALICE),
            INITIAL_BALANCE - amount
...
fn less_than_min_xcm_fee_should_not_work() {
    ...
    // so `transfer_multicurrencies` is not supported on Manta.
...
fn send_disabled_asset_should_fail() {
    ...
    ParaA::execute_with(|| {
        assert_ok!(AssetManager::update_outgoing_filtered_assets(
            parachain::RuntimeOrigin::root(),
            para_b_source_location,
        ));
        assert_ok!(parachain::Assets::mint_into(
