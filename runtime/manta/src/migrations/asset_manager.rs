...
impl<T: frame_system::Config + pallet_asset_manager::Config> OnRuntimeUpgrade for XcmV1ToV3<T> {
    fn on_runtime_upgrade() -> Weight {
        ...
        for (asset_id, v1_location) in asset_id_location {
            ...
            })) = v1_location
            {
                match interior {
                    ...
                    // { Parachain, PalletInstance, AccountKey20 }: 5
                    v1::Junctions::X3(
                        v1::Junction::Parachain(para_id),
                        v1::Junction::PalletInstance(pallet_index),
                        v1::Junction::AccountKey20 { key, .. },
                    ) => {
                        let v3_account_key20 = Junction::AccountKey20 { network: None, key };
                        let v3_junctions = Junctions::X3(
                            Junction::Parachain(para_id),
                            Junction::PalletInstance(pallet_index),
                            v3_account_key20,
        ...
        // MinXcmFee, no storage on manta

        // pallet version
        if crate::AssetManager::on_chain_storage_version() < 2 {
            StorageVersion::new(2).put::<pallet_asset_manager::Pallet<T>>();
            writes += 1;
            log::info!(target: "OnRuntimeUpgrade", "✅ AssetManager's version has been updated to 2.");
        }
        reads += 1;
    ...
    fn post_upgrade(_state: Vec<u8>) -> Result<(), sp_runtime::TryRuntimeError> {
        ...
        if crate::AssetManager::on_chain_storage_version() != 2 {
            return Err(DispatchError::Other(
                "AssetManager storage version is not 2, the migration wasn't executed.",
            ));
        }
