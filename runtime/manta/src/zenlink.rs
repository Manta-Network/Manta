...
use crate::assets_config::MantaConcreteFungibleLedger;
    ...
    fn generate_lp_asset_id(
        ...
        match (asset_id_0, asset_id_1) {
            (Some(asset_id0), Some(asset_id1)) => {
                let lp_asset_id: Option<MantaAssetId> =
...
impl LocalAssetAdaptor {
    #[cfg(not(feature = "runtime-benchmarks"))]
    fn asset_id_convert(asset_id: ZenlinkAssetId) -> Option<MantaAssetId> {
        ...
        let manta_asset_id = asset_id.asset_index as MantaAssetId;
        Some(manta_asset_id)
    }
    #[cfg(feature = "runtime-benchmarks")]
    fn asset_id_convert(asset_id: ZenlinkAssetId) -> Option<MantaAssetId> {
...
impl LocalAssetHandler<sp_runtime::AccountId32> for LocalAssetAdaptor {
    fn local_balance_of(asset_id: ZenlinkAssetId, who: &sp_runtime::AccountId32) -> AssetBalance {
        ...
        if let Some(manta_asset_id) = manta_asset_id {
            <MantaConcreteFungibleLedger as FungibleLedger>::balance(manta_asset_id, who)
        } else {
            AssetBalance::zero()
        }
    }
    ...
    fn local_total_supply(asset_id: ZenlinkAssetId) -> AssetBalance {
        let manta_asset_id = LocalAssetAdaptor::asset_id_convert(asset_id);
        if let Some(manta_asset_id) = manta_asset_id {
            <MantaConcreteFungibleLedger as FungibleLedger>::supply(manta_asset_id)
        } else {
            AssetBalance::zero()
        }
    }
    ...
    fn local_deposit(
        ...
        let manta_asset_id = LocalAssetAdaptor::asset_id_convert(asset_id);
        if let Some(manta_asset_id) = manta_asset_id {
            <MantaConcreteFungibleLedger as FungibleLedger>::deposit_minting(
                manta_asset_id,
                origin,
                amount,
            )
...
mod mock_benchmark {
    ...
    pub fn asset_id_convert(asset_id: ZenlinkAssetId) -> Option<MantaAssetId> {
        // Notice: Manta native asset id is 1, but Zenlink native asset id is 0.
        if asset_id.asset_index == ZenlinkNativeAssetId::get() {
            // When Zenlink asset index is 0, the asset type need to be NATIVE(0).
        ...
        Some(asset_id.asset_index as MantaAssetId)
    }
}
