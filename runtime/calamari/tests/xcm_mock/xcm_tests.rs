...
use xcm::{
    latest::prelude::{
        BuyExecution, Concrete, DescendOrigin, GeneralIndex, GeneralKey, Instruction, NetworkId,
        OriginKind, PalletInstance, Parent, QueryHolding, QueryResponse, Transact, Unlimited,
        WithdrawAsset, XcmError,
    },
    v2::Response,
    VersionedMultiLocation, WrapVersion,
};
...
fn xcmp_transact_from_sibling_tests() {
    ...
    let alice_derived_account_on_b =
        Account32Hash::<RelayNetwork, AccountId>::convert_ref(&MultiLocation {
            parents: 1,
            interior: X2(
                Parachain(PARA_A_ID),
                AccountId32 {
