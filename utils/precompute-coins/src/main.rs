...
use manta_xt::dolphin_runtime::runtime_types::pallet_manta_pay::types::TransferPost as MantaXtTransferPost;
...
async fn main() -> Result<()> {
    ...
    for i in 0..10 {
        let asset_id = (8 + (i % 6)).into();
        println!("Iteration count: {:?}, asset id: {:?}", i, asset_id);
        ...
        let encoded_coin = to_private.encode();

        let _m = MantaXtTransferPost::decode(&mut encoded_coin.as_slice()).unwrap();
        let mint_call = dolphin_runtime::tx().manta_pay().to_private(_m);
        let block = api
            .tx()
            .sign_and_submit_then_watch_default(&mint_call, &signer)
            .await
            .unwrap()
            .wait_for_finalized()
            .await
    ...
    // let mut file = File::create("./precomputed_mints_v1")?;
    // file.write_all(&<[TransferPost]>::encode(&mints))?;
    // let mut file = File::create("./precomputed_transfers_v1")?;
    // file.write_all(&<[TransferPost]>::encode(&transfers))?;
    // let mut file = File::create("./precomputed_reclaims_v1")?;
